...
export default function Product() {
    ...
    const handleSubmit = async () => {
        ...
        setForm({ name: "", image: "", category: "", price: "", discount: "", stock: "" });
        setEditId(null);
        setShowModal(false);
        fetchProducts();
    };
    ...
    return (
        <div style={{ padding: "20px" }}>
            ...
            <div style={topBar}>
                ...
                <button onClick={() => setShowModal(true)} style={addBtn}>+ Add Product</button>
                <button onClick={() => setShowCart(true)} style={cartBtn}>🛒 ({cart.length})</button>
            </div>
                ...
                {filteredProducts.map((p) => (
                    <div key={p._id} style={card}>
                        ...
                        <div style={{ padding: "10px" }}>
                            ...
                            <div style={{ display: "flex", gap: "5px", flexWrap: "wrap" }}>
                                <button onClick={() => addToCart(p)} style={addCartBtn} disabled={p.stock <= 0}>
                                    {p.stock <= 0 ? "Out of Stock" : "Add Cart"}
                                </button>
                                <button onClick={() => handleEdit(p)} style={editBtn}>Edit</button>
                                <button onClick={() => handleDelete(p._id)} style={delBtn}>Delete</button>
                            </div>
                        </div>
                    </div>
                ))}
            ...
            {showCart && (
                <div style={modal}>
                    <div style={modalBox}>
                                ...
                                {cart.map((item) => {
                                    ...
                                    return (
                                        <div key={item._id} style={cartItem}>
                                            <div>
                                                ...
                                                <div style={{ display: "flex", gap: "10px" }}>
                                                    ...
                                                    <button onClick={() =>
                                                        setCart((prev) =>
                                                            prev.map((p) =>
                                                                p._id === item._id && p.qty < item.stock
                                                                    ? { ...p, qty: p.qty + 1 } : p
                                                            )
                                                        )
                                                    }>+</button>
                                            ...
                                            <button onClick={() => removeFromCart(item._id)} style={delBtn}>X</button>
                                        </div>
                                    );
                                })}
                                <h3>Total: {grandTotal}</h3>
                        ...
                        <div style={{ display: "flex", gap: "10px", marginTop: "10px", flexWrap: "wrap" }}>
                            <button onClick={handleCheckout} style={addBtn}>Go to payment</button>
                            <button onClick={() => setShowCart(false)} style={delBtn}>Close</button>
                        </div>
                    </div>
                </div>
            )}
...
const topBar = { display: "flex", gap: "10px", marginBottom: "20px", flexWrap: "wrap" };
const input = { padding: "10px", flex: "1 1 200px", borderRadius: "6px", border: "1px solid #ddd" };
const addBtn = { padding: "10px 15px", background: "#0d6efd", color: "#fff", border: "none", borderRadius: "6px" };
const cartBtn = { padding: "10px 15px", background: "#000", color: "#fff", border: "none", borderRadius: "6px" };
const addCartBtn = { background: "green", color: "#fff", border: "none", padding: "6px 10px", borderRadius: "5px" };
const editBtn = { background: "#ffc107", border: "none", padding: "6px 10px", borderRadius: "5px" };
const delBtn = { background: "#dc3545", color: "#fff", border: "none", padding: "6px 10px", borderRadius: "5px" };
const grid = { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "15px", maxHeight: "70vh", overflowY: "auto", paddingRight: "5px" };
const card = { background: "#fff", borderRadius: "10px", overflow: "hidden", boxShadow: "0 4px 10px rgba(0,0,0,0.08)" };
const img = { width: "100%", height: "150px", objectFit: "cover" };
const modal = { position: "fixed", top: 0, left: 0, width: "100%", height: "100%", background: "rgba(0,0,0,0.5)", display: "flex", justifyContent: "center", alignItems: "center" };
const modalBox = { background: "#fff", padding: "20px", borderRadius: "10px", width: "90%", maxWidth: "400px", maxHeight: "90vh", overflowY: "auto" };
const cartItem = { display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "10px", borderBottom: "1px solid #eee", paddingBottom: "10px", marginBottom: "10px" };